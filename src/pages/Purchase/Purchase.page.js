...
const Purchase = (props) => {
  ...
  return (
    <>
      <Helmet>
        <title>فروشگاه ساکورا | خرید </title>
      </Helmet>
      {!!data.length &&
        data.map((item, index) => (
          <div key={index}>
            <Box className={classes.box} key={item.firstName}>
              ...
              <Box className={classes.infoBox}>
                <Typography
                  ...
                  component="h1"
                >
                  {item.category.name} {item.firstName}{" "}
                </Typography>
                ...
                {+item.count < 5 && +item.count !== 0 && (
                  <Typography sx={{ color: "#c0ca33" }}>
                    تنها {item.count} عدد از کالا باقی مانده است
                  </Typography>
                )}
              </Box>
            </Box>
            <Box className={classes.figureGroup}>
              {item.image.map((image, index) => (
                <figure
                  key={index}
                  onClick={(e) =>
                    handleOpen(`http://localhost:3002/files/${image}`)
                  }
                  style={{
                    ...
                    {item.image.map((image, index) => (
                      <figure
                        ...
                        style={{
                          ...
                          cursor: "pointer",
                          border:
                            `http://localhost:3002/files/${image}` === mainImage
                              ? "2px solid #3e9fd8"
                              : "2px solid #ccc",
                        }}
                        onClick={(e) =>
                          setMainImage(`http://localhost:3002/files/${image}`)
                        }
                      >
                        <CardMedia
                          sx={{ width: "70px!important" }}
