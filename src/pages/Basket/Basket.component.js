...
const Basket = (props) => {
  ...
  const didMount = () => {
    ...
    storage === null
      ? (storage = [])
      : (storage = JSON.parse(localStorage.getItem("orders")));
    localStorage.setItem("orders", JSON.stringify(storage));

    setData(storage);

    setTotalPrice(
      storage.reduce((total, item) => total + item.price * item.count, 0)
    );
  };
  ...
  const handlePlusOrMines = (e, target, index, orderId) => {
    http.get(PRODUCTS + "/" + orderId).then((res) => {
      ...
      if (target === "plus") {
        storage[index].count++;
        storage[index].totalAmount =
          storage[index].price * storage[index].count;
        localStorage.setItem("orders", JSON.stringify(storage));
        setData(storage);
        setTotalPrice(
          storage.reduce((total, item) => total + item.price * item.count, 0)
  ...
  return (
    <>
      <Helmet>
        <title>فروشگاه ساکورا | سبد خرید </title>
      </Helmet>
      <Typography sx={{ fontFamily: "Vazir-bold" }} variant="h4" component="h3">
        {" "}
        سبد خرید{" "}
      </Typography>
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                {columns.map((column) => (
                  <StyledTableCell
                    sx={{ fontSize: "1.1rem", cursor: column.cursor }}
                    key={column.id}
                    align={column.align}
              ...
              {!!data.length &&
                data.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <PersianNumber number={item.price} />
                      &nbsp;تومان
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        <Button
                          ...
                          onClick={(e) =>
                            handlePlusOrMines(e, "plus", index, item.orderId)
                          }
                        >
                          +
                        </Button>
                        ...
                        <Button
                          ...
                          sx={{ minWidth: "40px", height: "40px" }}
                          onClick={(e) =>
                            handlePlusOrMines(e, "mines", index, item.orderId)
                          }
                        >
                          -
                        </Button>
      ...
      >
        ...
        >
          ...
          <Typography
            ...
            variant="h5"
          >
            <PersianNumber number={totalPrice} />
            &nbsp; تومان{" "}
          </Typography>
        </Box>
        <Button
          disabled={data.length === 0}
          onClick={(e) => handleBasket()}
