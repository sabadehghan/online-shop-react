...
const PaymentResult = () => {
  ...
  const didMount = () => {
    if (status === "success") {
      const storageDataCustomer = JSON.parse(
      ...
      storageDataOrders.forEach((orderInfo) => {
        http.get(`${PRODUCTS}/${orderInfo.orderId}`).then((res) => {
          ...
          http
            .patch(`${PRODUCTS}/${orderInfo.orderId}`, {
              count: orderCount - orderInfo.count,
            })
            .then((res) => {
              console.log(res);
            });
      ...
      const data = {
        name:
          storageDataCustomer.firstName + " " + storageDataCustomer.lastName,
        date: moment().locale("fa").format("YYYY/MM/DD"),
        status: "doing",
        address: storageDataCustomer.address,
        totalAmount: storageDataOrders.reduce(
  ...
  return (
    <>
      <Helmet>
        <title> فروشگاه ساکورا | نتیجه پرداخت </title>
      </Helmet>
      {status === "failure" ? (
        <>
          <Typography
            sx={{ fontFamily: "Vazir-bold" }}
            variant="h4"
            component="h6"
          >
            {" "}
