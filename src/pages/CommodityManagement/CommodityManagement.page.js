...
const CommodityManagement = () => {
  ...
  const [page, setPage] = useState(
    searchURL[1] ? searchURL[1].split("=")[1] : 1
  );
  ...
  const fetchProducts = async () => {
    ...
    const { data } = await http.get(
      `${CUSTOMERS}?status=${value}&_sort=deliverytime&_order=desc&_page=${page}&_limit=10`
    );
    setData(data);
              ...
              {!!data.length
                ...
                : skeletonCount.map((item, index) => (
                    <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                      ...
                      <TableCell sx={{ width: "20%" }}>
                        <Skeleton animation="wave" variant="rect" width={50} />
                      </TableCell>
                      <TableCell sx={{ width: "40%" }}>
                        <Skeleton animation="wave" variant="rect" width={50} />
                      </TableCell>
        ...
        <Fade in={open}>
          <Box className={classes.modalBox}>
                  ...
                  <Box key={item.id}>
                    <article>
                      <p style={{ lineHeight: "2rem" }}>
                        <span style={{ fontFamily: "Vazir-bold" }}>
                          {" "}
                          نام مشتری:
                        </span>{" "}
                        {item.name} <br />
            ...
            {!!dataModal.length && dataModal[0].status === "done" ? (
              <Button
                onClick={(e) => handleChangeStatus(dataModal[0].id, "done")}
                color="warning"
                variant="contained"
              >
                {" "}
                تغیر وضعیت به سفارش های درحال انتظار{" "}
              </Button>
            ) : (
              <Button
                onClick={(e) => handleChangeStatus(dataModal[0].id, "doing")}
                color="success"
                variant="contained"
              >
                {" "}
                تغیر وضعیت به سفارش های تحویل داده شده{" "}
              </Button>
            )}
          </Box>
        </Fade>
