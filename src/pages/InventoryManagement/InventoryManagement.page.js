...
const InventoryManage = (props) => {
  ...
  const [page, setPage] = useState(
    searchURL[2] ? searchURL[2].split("=")[1] : 1
  );
  const [numberOfPages, setNumberOfPages] = useState(10);
  const [value, setValue] = React.useState(
    searchURL[0] ? searchURL[0].split("=")[1] : ""
  );
  ...
  const fetchProducts = async () => {
    navigate(
      `?_sort=${value}&_order=${
        value === "brand" ? "asc" : "desc"
      }&_page=${page}&_limit=10`
    );
    const { data } = await http.get(
      `${PRODUCTS}?_sort=${value}&_order=${
        value === "brand" ? "asc" : "desc"
      }&_page=${page}&_limit=10`
    );
    setData(data);
  ...
  return (
    <>
      <Helmet>
        <title>فروشگاه ساکورا | مدیریت سفارش ها </title>
      </Helmet>
              ...
              {!!data.length
                ? data.map((item) => (
                    <TableRow key={item.id} hover role="checkbox" tabIndex={-1}>
                      ...
                      <TableCell
                        onClick={(e) => handlePrice_Count(e, "price", item.id)}
                        className={classes.changer}
                      >
                        <span
                          style={{
                            display:
                              !!displayInputPrice.length &&
                              displayInputPrice.includes(item.id)
                                ? "none"
                                : "block",
                      ...
                      <TableCell
                        onClick={(e) => handlePrice_Count(e, "count", item.id)}
                        className={classes.changer}
                      >
                        <span
                          style={{
                            display:
                              !!displayInputCount.length &&
                              displayInputCount.includes(item.id)
                                ? "none"
                                : "block",
                            fontFamily: "Vazir-bold",
                          }}
...
const InventoryManagement = connect(
  undefined,
  mapDispatchToProps
)(InventoryManage);
